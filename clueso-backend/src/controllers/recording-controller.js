...
exports.processRecording = async (req, res) => {
  try {
    ...
    Logger.info(`[Recording Controller] Processing recording for project: ${projectId}, session: ${sessionId}`);
    ...
    // Update recording session in Supabase if project info is available
    if (projectId && sessionId) {
      try {
        const ProjectService = require('../services/project-service');
        
        // Update recording session with file paths
        await ProjectService.updateRecordingSession(sessionId, {
          file_path: permanentVideoPath || permanentAudioPath,
          metadata: {
            ...metadata,
        ...
        // Save file records
        await ProjectService.saveRecordingFiles(sessionId, permanentVideoPath, permanentAudioPath);
        
        Logger.info(`[Recording Controller] Updated project recording session: ${sessionId}`);
      } catch (projectError) {
        Logger.error(`[Recording Controller] Error updating project data:`, projectError);
        // Don't fail the entire process if project update fails
      }
    }

    // Start automatic video processing if video exists and project is specified
    if (permanentVideoPath && projectId && sessionId) {
      try {
        ...
        Logger.info(`[Recording Controller] Starting automatic video processing for session: ${sessionId}`);
        ...
        VideoProcessingService.processVideoUpload(
          ...
          permanentAudioPath,
          {
            ...metadata,
            autoProcessed: true,
            recordingEvents: events.length
          },
          userId
        ).catch(error => {
          Logger.error(`[Recording Controller] Background video processing failed:`, error);
        });
        ...
        Logger.error(`[Recording Controller] Error starting video processing:`, processingError);
        result.videoProcessing = {
          status: 'failed',
    ...
    // Store transcription in AI analysis table if project info is available
    if (transcriptionResult && projectId && sessionId) {
      try {
        ...
        await supabaseAdmin
          ...
          .insert([{
            ...
            result: {
              ...
              deepgram_response: transcriptionResult.deepgramResponse
            },
            confidence_score: transcriptionResult.deepgramResponse?.metadata?.confidence || null
          }]);
        
        Logger.info(`[Recording Controller] Saved transcription analysis for session: ${sessionId}`);
      } catch (analysisError) {
        Logger.error(`[Recording Controller] Error saving transcription analysis:`, analysisError);
      }
    }
    ...
    if (transcriptionResult && transcriptionResult.text) {
      ...
      // Store AI processing results if available
      if (pythonResponse && projectId && sessionId) {
        try {
          const { supabaseAdmin } = require('../config/supabase');
          await supabaseAdmin
            .from('ai_analysis')
            .insert([{
              recording_session_id: sessionId,
              analysis_type: 'ai_processing',
              result: pythonResponse,
              confidence_score: null // Python response doesn't include confidence
            }]);
          
          Logger.info(`[Recording Controller] Saved AI processing analysis for session: ${sessionId}`);
        } catch (analysisError) {
          Logger.error(`[Recording Controller] Error saving AI processing analysis:`, analysisError);
        }
      }
    ...
    result.recordingSessionId = sessionId;
