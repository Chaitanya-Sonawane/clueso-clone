...
// Test collaboration session creation without authentication
router.post('/collaboration-session', (req, res) => {
    try {
        const { videoId, sessionName } = req.body;
        const sessionId = `test_session_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        
        Logger.info(`[Test Route] Creating test collaboration session: ${sessionId} for video: ${videoId}`);
        
        // Mock session data
        const session = {
            id: sessionId,
            videoId: videoId || 'demo_video_123',
            sessionName: sessionName || 'Test Collaboration Session',
            createdBy: 'test_user',
        ...
        res.status(201).json({
            success: true,
            message: 'Test collaboration session created successfully',
            data: session
        });

    } catch (error) {
        Logger.error(`[Test Route] Error creating test session:`, error);
        res.status(500).json({
            success: false,
            message: error.message
        });
